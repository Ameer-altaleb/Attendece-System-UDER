...
const Dashboard: React.FC = () => {
  ...
  const activeCenterIds = useMemo(() => activeCentersList.map(c => c.id), [activeCentersList]);
  
  // فلترة الموظفين والسجلات التابعين للمراكز النشطة فقط
  const activeEmployees = useMemo(() => employees.filter(e => activeCenterIds.includes(e.centerId)), [employees, activeCenterIds]);
  const todayRecords = useMemo(() => attendance.filter(a => a.date === today && activeCenterIds.includes(a.centerId)), [attendance, today, activeCenterIds]);

  // حساب الإحصائيات المتقدمة بناءً على الفلاتر النشطة
  const stats = useMemo(() => {
  ...
  // بيانات مخطط الحالة (Pie Chart) - مراكز نشطة فقط
  ...
  // بيانات مخطط النشاط الأسبوعي
  const weeklyData = useMemo(() => {
    return Array.from({ length: 7 }).map((_, i) => {
      const date = format(subDays(new Date(), 6 - i), 'yyyy-MM-dd');
      const count = attendance.filter(a => a.date === date && activeCenterIds.includes(a.centerId)).length;
      return {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-1000">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
        {stats.map((stat, i) => (
          <div key={i} className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 relative overflow-hidden group hover:border-indigo-200 transition-all">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
        ...
        <div className="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm">
          ...
          <div className="space-y-4">
            {todayRecords.slice(0, 5).reverse().map((record, i) => {
              ...
              return (
                <div key={i} className="flex items-center justify-between p-4 bg-slate-50 rounded-3xl border border-slate-100 hover:bg-white hover:shadow-md transition-all">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="text-xs font-black text-slate-800">{emp?.name || 'موظف غير معروف'}</p>
                      <p className="text-[9px] text-slate-400 font-bold uppercase">{record.checkOut ? 'تسجيل انصراف' : 'تسجيل حضور'}</p>
                    </div>
                  </div>
                  <div className="text-left">
                    <p className="text-xs font-black text-slate-900">{record.checkOut ? format(new Date(record.checkOut), 'HH:mm') : format(new Date(record.checkIn!), 'HH:mm')}</p>
        ...
        <div className="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm">
          ...
          <div className="space-y-4">
            {activeCentersList.map((center, i) => {
              ...
              return (
                <div key={i} className="space-y-2">
                  <div className="flex items-center justify-between px-2">
                    ...
                    <span className="text-[10px] font-black text-slate-400">{centerRecords} / {centerEmps} موظف</span>
                  </div>
